...
TIMELINE_TAB_BUTTON = ('<button type="button" data-tab-target="timeline">'
                       + icon("trend") + "<span>时间线</span></button>")
...
def render_section(rows: list[dict], meta: list[dict]) -> str:
    ...
    out.append("</ul>")
    out.append('<div class="table-wrap"><table><thead><tr><th>发生时间</th><th>流</th><th>类型</th><th>主体</th><th>版本</th><th>字段</th><th>ID</th></tr></thead><tbody>')
    for e in rows:
        fields = "; ".join(
