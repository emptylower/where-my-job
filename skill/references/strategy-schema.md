...
**城市不是白名单。** 内置合肥、上海、北京、深圳、广州、杭州、武汉七个便利名；其余城市直接写平台城市码（形如 `101270100`），或在 `city_codes` 里给出名字到码的映射再按名字用。`city_codes` 优先于内置名。反查不到名字时，dry-run 的 `city` 显示码本身，不编名字。

**CLI 不内置全国城市表**：随包发一张没有可信来源的表，错了就是**静默错值**，且随发布固化、谁都查不出来。这条约束管的是 CLI，不是 agent——**内置名之外的码由 agent 自己检索并交叉核对后填进 `city_codes`，不要转手让用户去查**（规则见 SKILL.md「采集范围」）。区别在于：agent 是逐次查、当场核对、把来源说给用户听；写死进包里的表没有这三样。
